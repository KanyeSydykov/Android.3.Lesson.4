...
public class WeatherFragment extends BaseFragment<FragmentWeatherBinding> {
    ...
    private void showModel(MyWeather model) {
        Log.d("WeatherFragment", "showModel: " + model.getName());
        ...
        String dateRise = new java.text.SimpleDateFormat("hh:mm a").format(new Date(sunriseSecs * 1000));
        binding.textSunRiseTimeFragmentWeather.setText(dateRise);

        int sunsetSecs = model.getSys().getSunset();
        @SuppressLint("SimpleDateFormat")
        String dateSet = new java.text.SimpleDateFormat("hh:mm a").format(new Date(sunsetSecs * 1000));
